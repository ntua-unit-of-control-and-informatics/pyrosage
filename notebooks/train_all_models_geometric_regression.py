...
def train_and_evaluate(model_name, hyperparams):
    ...
    fig = plot_scatter(final_test_targets, final_test_preds, title=f'Predicted vs Actual (Test) - {model_name}')
    fig.savefig(f"../plots/{model_name}_scatter_plot.png")
    
    # 3. Residuals plot
    fig = plot_residuals(final_test_targets, final_test_preds, title=f'Residuals Plot (Test) - {model_name}')
